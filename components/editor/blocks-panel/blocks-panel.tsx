...
import { Layout, Type, Image, Video, LinkIcon, Grid, Columns } from "lucide-react"
...
export function BlocksPanel({ blocks, onAddBlock }: BlocksPanelProps) {
  ...
  const categories = blocks.reduce(
    (acc, block) => {
    ...
    <div className="space-y-4">
      {Object.entries(categories).map(([category, categoryBlocks]) => (
        <div key={category} className="space-y-2">
          <h3 className="text-xs font-medium text-slate-300">{formatCategoryName(category)}</h3>
          <div className="grid grid-cols-2 gap-2">
            {categoryBlocks.map((block) => (
              <Card
                ...
                onClick={() => {
                  const content =
                    typeof block.content === "string" ? block.content : `<div data-gjs-type="${block.id}"></div>`
                  onAddBlock(content)
                }}
              >
