...
import { Search, Save, FileText, Download, Upload } from "lucide-react"
...
export function TemplateManager({ onSelectTemplate, onSaveTemplate, currentContent }: TemplateManagerProps) {
  ...
  const handleAddSelectedTemplates = () => {
    ...
    const templates = componentTemplates.filter((template) => selectedTemplates.includes(template.id))
        ...
        <div className="flex items-center space-x-2 mb-4">
          ...
          <Dialog open={saveDialogOpen} onOpenChange={setSaveDialogOpen}>
            ...
            <DialogContent className="bg-slate-900 border-slate-700">
              <DialogHeader>
                <DialogTitle className="text-slate-100">Save as Template</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <label htmlFor="template-name" className="text-xs font-medium text-slate-300">
                    Template Name
                  </label>
                  <Input
                    id="template-name"
                ...
                <div className="flex justify-end space-x-2">
                  <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => setSaveDialogOpen(false)}>
                    Cancel
                  </Button>
                  <Button size="sm" className="h-8 text-xs" onClick={handleSaveTemplate}>
                    Save Template
                  </Button>
                </div>
              </div>
        ...
        <Tabs defaultValue="browse" className="flex-1 flex flex-col min-h-0">
                  ...
                  {Object.entries(groupedTemplates).map(([category, templates]) => (
                    <div key={category}>
                      <h3 className="text-sm font-medium mb-3 text-slate-200 sticky top-0 bg-slate-900 py-2 z-10">
                        {componentCategories.find((c) => c.id === category)?.label || category}
                      </h3>
                          ...
                          >
                            <CardContent className="p-2">
                              <div className="aspect-video bg-slate-700 rounded-sm mb-2 overflow-hidden">
                                <img
                                  src={template.thumbnail || "/placeholder.svg?height=100&width=200"}
                                  alt={template.label}
                                  className="w-full h-full object-cover"
                                />
                              </div>
                              <h4 className="font-medium text-xs text-slate-200 truncate">{template.label}</h4>
                            </CardContent>
                          </Card>
                        ))}
                      </div>
            ...
            {selectedTemplates.length > 0 && (
              <div className="sticky bottom-0 bg-slate-900 p-3 border-t border-slate-700 mt-4">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-300">{selectedTemplates.length} template(s) selected</span>
                  <Button size="sm" className="h-8 text-xs" onClick={handleAddSelectedTemplates}>
          ...
          <TabsContent value="saved" className="flex-1 min-h-0">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-medium text-slate-200">My Saved Templates</h3>
            ...
            <div className="flex-1 flex items-center justify-center text-center p-6 text-slate-400 border-2 border-dashed border-slate-700 rounded-lg">
              <div>
                ...
                <h4 className="text-sm font-medium mb-2">No saved templates yet</h4>
                <p className="text-xs mb-4">Save your current design as a template to reuse it later</p>
                <Button size="sm" className="h-8 text-xs" onClick={() => setSaveDialogOpen(true)}>
                  <Save className="w-3.5 h-3.5 mr-1.5" />
                  Save Current Design
                </Button>
              </div>
